...
func resourceIBMContainerALBCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOkExists("enable"); ok {
		...
		return fmt.Errorf("Provide either `enable` and `disable_deployment`")
	}
	...
	d.SetId(albID)
	_, err = waitForContainerALBCreate(d, meta, albID, enable, disableDeployment)
	if err != nil {
		return fmt.Errorf(
			"Error waiting for create resource alb (%s) to be enabled: %s", d.Id(), err)
	}
...
func waitForContainerALBCreate(d *schema.ResourceData, meta interface{}, albID string, enable, disableDeployment bool) (interface{}, error) {
	albClient, err := meta.(ClientSession).ContainerAPI()
	if err != nil {
		return false, err
	}
